...
const NewPostForm = ({ onSubmit, isSubmitted }) => {
  ...
  return (
    ...
    >
      {(formik) => (
        <Col>
          <Card className="boxShadow border-rounded fs-4 bg-info ">
            <Card.Header className=" text-center fs-4 border m-2 bg-warning">{`${t("newPostBy")} ${
              authCtx.userName
            }`}</Card.Header>
            <Card.Body className="m-2 pb-3 border rounded-3">
              <Form onSubmit={formik.handleSubmit}>
                ...
                <Form.Group className="my-2" controlId="content">
                  ...
                  <Form.Control
                    type="text"
                    placeholder={t("Your Experience, Tips, Advices, etc")}
                    name="content"
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
